...
// Global variables
// Editable
TString sim_data_input = "data/neutrons/0_0.root"; 
...
void anger(){
    ...
    Int_t threshold = 5;  // Modificar el numero minimo de cuentas a tener en cuenta.
    Int_t good_number_to_run = 16;   // Editar el numero minimo de PMTs con los que se correrá el codigo.
...
//    gStyle->SetPadBottomMargin(0.1);
    ...
    TCanvas *c2 = new TCanvas("c2", "X_error",800,800);
    gStyle->SetOptStat(0);
    TH1F *h1 = new TH1F("h1","X_error ; dx (cm) ; No. muestras",300,0,30);
    tree->Draw("dx>>h1","");
    h1->Draw();

    c2->SaveAs("X_error.png");

    TCanvas *c3 = new TCanvas("c3", "Y_error",800,800);
    gStyle->SetOptStat(0);
    TH1F *h2 = new TH1F("h2","Y_error ; dy (cm) ; No. muestras",300,0,30);
    tree->Draw("dy>>h2","");
    h2->Draw();

    c3->SaveAs("Y_error.png");
